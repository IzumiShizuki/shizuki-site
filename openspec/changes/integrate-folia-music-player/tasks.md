...
- [x] 主题深度桥接（站点 accent/壁纸 → Folia 视觉参数）
...
## 普通模式播放回归修复（2026-09-15）

- [x] **失效音源自愈与站内交付**：普通播放器在 `play()` 被拒绝时强制刷新地址并重试；解析结果改用支持 `Range` 的短时加密站内流地址，绕开浏览器直连第三方 CDN 的网络中断；补充前后端回归测试并完成线上验证。

## 播放会话交互回归（2026-09-17）

- [x] **Folia 动态歌词与双向 seek**：跟随主站时同步 Folia 的歌词专用时钟；拦截 Folia 进度条输入并把 seek 命令回传主站，验证歌词高亮与普通播放器进度保持一致。
...
- [x] **主歌词稳定渲染**：修复 Folia 嵌入态主歌词在切行时闪烁或消失的问题，保留翻译为次级轨道。
- [x] **工作区融合与队列保持**：移除独立模式切换入口；在 Folia 中浏览/切歌时始终保留主站完整队列和下一首顺序。
- [x] **Home 壁纸与全屏视觉**：向 Folia 同步 Home 壁纸，常规视图实现模糊背景与清晰音乐内容区，全屏时全域高清。
- [x] **普通模式频谱校准**：平衡高低频并加入软限幅，避免小幅音量提高后频谱持续顶格。

## 控制与音源回归（2026-09-17）

- [x] **普通模式账号音源即时同步**：Folia 最新网易云 Cookie 回写后，普通模式无需等待账号状态投影刷新即可优先使用授权音源；补充回归测试。
- [x] **Folia 随机跳曲与即时拖动**：Folia 上一首/下一首委托主播放器的播放模式；进度条按下/拖动立即 seek，不依赖移出鼠标。
- [x] **Folia 歌单浏览入口**：歌单详情能够以 Folia lattice 浏览完整歌单，并保留站点完整队列。
...
- [x] **生产验收**：目标曲 `1880877106` 在普通模式解析为 `247766 ms` 完整流；首段 Range 返回 `206 bytes 0-65535/9912991`，按 35 秒计算的偏移 `1400332` 返回 `206 bytes 1400332-1465867/9912991`，确认实际音频可越过 30 秒。

## Folia 站内体验收尾（2026-09-22）

- [x] **当前歌曲精确进入**：歌曲入口先解析并选中 `provider + 原始 trackId` 对应曲目，再显示 Folia `player` 视图；冷启动期间保留最后一次视图请求。
- [x] **歌词单次平滑切行**：移除活动行索引触发的整会话重发，Folia 复用未变化的歌词时间线，并由连续时钟唯一推进焦点。
- [x] **网站视口铺满**：移除浏览器 Fullscreen API，使用同一 Folia DOM 树铺满网站可视区域，保留浏览器自身界面并支持按钮、返回音乐库与 `Esc` 还原。
- [x] **展开态长歌词防裁切**：移除展开时跳过歌词适配的分支；Cadenza 主词提供可测量标记，桥以变换后 DOM 联合宽度优先适配，未挂载时才以约 188px 基准、完整活动行的加权 grapheme 宽度、用户字号和活动词强调安全倍率估算；取消会阻止超长行继续缩小的固定下限，并在投影时钟切换活动行后的下一帧重新适配，补充 Folia 歌词桥接回归测试。
- [x] **Classic/Partita 长歌词实测补全**：生产验收发现默认 Classic 主歌词不经过 Cadenza overlay，导致 DOM 实测选择器为空；为 Classic、Partita 实际词节点补充统一测量标记，并把三种可视化路径纳入回归测试。
- [x] **歌词缩放审计去重与恢复**：生产验收发现换句审计与缩放函数会在同一动画帧重复测量旧 DOM、累积缩小到异常字号；按当前活动行限定测量范围，将审计合并为每帧一次，稳定居中后允许字号按真实剩余空间回升，离场/入场偏移帧保持上一字号。
- [x] **歌词入场几何稳定门控**：生产连续采样复现新句在 Framer Motion 弹簧入场时被误缩到 `5.728px`；活动行未挂载时不再回退扫描旧 DOM，入场阶段使用不受 transform 影响的布局宽度，连续稳定后才采用视觉边界，并延长审计覆盖完整弹簧过渡。
- [x] **动态高亮缩放下限**：精确 seek 回归发现词语高亮 transform 在容器稳定后仍会改变视觉半宽，个别句子可继续降到 `1.168px`；以完整行的 transform-independent 布局宽度作为视觉测量的安全下限，动画边界只允许在有余量时放大字号。
- [x] **2026-09-24 生产验收**：Folia 源码 SHA-256 与本地验证版本一致；`folia-gateway` 重建后状态为 `healthy`，`/healthz` 返回 `{"ok":true,"service":"folia-gateway"}`；线上主资源包含活动行/活动词标记与 `shizuki:follow-playback` 桥接逻辑。
