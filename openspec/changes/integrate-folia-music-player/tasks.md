...
- [ ] **主歌词稳定渲染**：修复 Folia 嵌入态主歌词在切行时闪烁或消失的问题，保留翻译为次级轨道。
- [ ] **工作区融合与队列保持**：移除独立模式切换入口；在 Folia 中浏览/切歌时始终保留主站完整队列和下一首顺序。
- [ ] **Home 壁纸与全屏视觉**：向 Folia 同步 Home 壁纸，常规视图实现模糊背景与清晰音乐内容区，全屏时全域高清。
- [ ] **普通模式频谱校准**：平衡高低频并加入软限幅，避免小幅音量提高后频谱持续顶格。
