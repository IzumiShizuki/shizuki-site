...
- **主歌词不降级为翻译**：嵌入态以主站 `lyrics` 时间线作为唯一歌词数据。主歌词层与底部翻译层分别渲染；切换活动行只更新焦点和动画状态，不清空或重新创建整个歌词树。没有时间轴时保留静态主歌词，不以翻译替代主歌词。
- **Folia 是音乐工作区功能**：移除将普通/Folia 描述为两个独立模式的顶栏切换。歌单、搜索和队列在同一个音乐工作区中持续可用；沉浸歌词仅改变当前内容呈现，不能缩减或重建主站队列。
- **壁纸层次**：站点向 Folia 推送当前 Home 壁纸 URL。常规沉浸视图以低清/模糊壁纸填满工作区背景，并在音乐内容区域使用同一张不模糊的高清壁纸；进入全屏时取消背景模糊，让高清壁纸覆盖整个沉浸画面。无有效壁纸时保留 Folia 默认视觉。
- **频谱动态范围**：普通模式采用对数频带重采样、低频衰减与软限幅，把低频、高频都保持可见；输出幅度基于压缩后的能量而非音量线性放大，避免小幅提高音量即顶格。

## 控制权与音源即时同步（2026-09-17）

- **跳曲控制权**：Folia 的上一首/下一首按钮在展示 store 切换前被识别为导航意图，并由主站 `usePlayerEngine.playNext/playPrev` 执行。Folia 的本地展示队列永远不决定随机顺序；歌曲卡片的直接选择仍作为指定曲目的播放意图处理。
...
- **展开态长歌词适配**：展开只增加可用工作区，不能关闭嵌入歌词的宽度保护。Cadenza 主词节点提供稳定标记，桥优先读取其变换后的联合 DOM 宽度，按根容器两侧留白换算临时字号；DOM 尚未挂载时才以完整活动行的加权 grapheme 宽度、约 188px 主层基准、用户字号和活动词强调安全倍率兜底。活动行由投影时钟改变后，在下一帧重新测量新节点，不能沿用上一句的临时字号。临时字号缩放不持久化，且不得以固定最小倍率阻止超长的中英文、日文混排行缩小到站内视口以内。
