...
- [ ] 6.3 Integrate only non-ancestor, non-superseded unique work from the desktop, music subtitle/HTTPS, wallpaper and Qianji branches, resolving conflicts from their primary specs and running focused tests after each stage.
- [ ] 6.4 Run final frontend/backend/OpenSpec gates on a clean integration result, update local master from fetched `origin/master`, merge or fast-forward the validated integration result, and push master.
