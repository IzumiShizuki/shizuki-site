...
关键配置：
- 配置入口：`apps/monolith-app/src/main/resources/application.yml`
- 外部配置：`resouces/yaml/monolith.yaml` + `resouces/yaml/common-config.yaml`
- 白名单路径（登录、公开资源、健康检查）
- 管理路由需显式配置到入口鉴权层：
  - `/api/v1/admin/users/**`
  - `/api/v1/admin/groups/**`
...
- 运行时迁移以 `apps/monolith-app/src/main/resources/monolith/db/migration/V*.sql` 为准。
- 辅助 SQL 与说明视图保留在 `resouces/sql/*.sql`。
- 未发布环境可按需求并入基线脚本，但需保持运行时迁移目录与 `resouces/sql` 同步。
...
1. 父工程 + libs 公共库完成
2. monolith-app + user-module 打通登录链路
...
1. [已完成] `pom.xml`：已改为多模块父工程并引入 MyBatis-Plus、Sa-Token 等依赖管理。
2. [已完成] `compose.yaml`：已补齐中间件与 monolith 容器模板。
3. [已完成] 配置体系：已迁移为 `application.yml + resouces/yaml/monolith.yaml + common-config.yaml` 模式。
