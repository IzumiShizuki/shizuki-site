...
- 适用于本仓库所有模块：`apps/monolith-app`、`modules/*`、`libs/*`、`resouces/*`、`scripts/*`。
- 本文优先级高于“个人习惯”，低于“明确产品需求”与“安全要求”。

### 0.1 本地工具链（当前开发机默认）

- JDK：`C:\Program Files\Java\`（建议使用 JDK 17）
- Maven：`D:\programFile\apache-maven-3.9.9\`
- 若未配置环境变量，AI 给出的命令应优先使用上述本地绝对路径或在说明中明确对应路径。
...
- 运行时数据库迁移以 Flyway 为准：`apps/monolith-app/src/main/resources/monolith/db/migration/*.sql`。
...
- 密钥禁止写死在代码中；默认从环境变量与 `resouces/yaml/common-config.yaml` 读取（模板见 `common-config.example.yaml`）。
