...
### 3.1 绑定冲突确认流程（Mermaid）

```mermaid
flowchart TD
    A[OAuth 登录返回 BIND_REQUIRED + bind_ticket] --> B[用户输入冲突邮箱账号与密码]
    B --> C[POST /api/v1/auth/bindings/confirm]
    C --> D[读取并校验 bind_ticket]
    D --> E{ticket 有效且未过期?}
    E -->|否| E1[返回 400 BAD_REQUEST]
    E -->|是| F[校验邮箱与密码归属目标账号]
    F --> G{凭据匹配?}
    G -->|否| G1[返回 401 UNAUTHORIZED]
    G -->|是| H[检查 provider_user_id 是否已绑定]
    H --> I{已被其他账号绑定?}
    I -->|是| I1[返回 400 BAD_REQUEST]
    I -->|否| J[创建或确认 OAuth 绑定]
    J --> K[更新 OAU_LOGIN 为 SUCCESS]
    K --> L[消费 bind_ticket]
    L --> M[签发 Access JWT + Refresh Token]
    M --> N[返回 TOKEN_ISSUED]
```
...
- `modules/user-module/src/main/java/io/github/shizuki/site/user/controller/MeController.java`
...
运行时迁移入口：
- `apps/monolith-app/src/main/resources/monolith/db/migration/*.sql`

变更内容：
- `V403__add_user_avatar_url.sql`（用户头像字段）
...
- `modules/user-module/src/main/resources/application.yml`
  - `shizuki.auth.jwt.*`
  - `shizuki.auth.captcha.*`
  - `shizuki.auth.captcha.image-max-attempts`
  - `shizuki.auth.verify.*`
  - `shizuki.auth.bind.*`
  - `shizuki.security.secret.enforce`
  - `spring.mail.*`
...
- `modules/ai-module/src/main/resources/application.yml`
...
- JWT 密钥改为配置驱动：`shizuki.auth.jwt.secret=${AUTH_JWT_SECRET:${random.uuid}}`，并通过启动校验器控制是否强制拦截。
- 图形验证码新增失败次数上限（默认 5 次），超过阈值自动失效并需重新获取。
- 审计 outbox 增加 `PROCESSING` 超时回收，避免实例异常退出导致事件卡死。
- 网关相关日志已统一中文描述，同时保留 `reason/upstream_status/path/guest_path/policy` 结构化字段。
- `libs/common-servlet` 业务日志切面新增 `BIZ_START`（auth/me 请求范围）。
- 管理员二次权限码校验原因码统一：`ADMIN_PRIVILEGE_REQUIRED`。
...
```bash
mvn -pl libs/common-core,libs/common-integration,modules/ai-module,modules/user-module -am test -DskipITs -Dtest=SpringRetryExecutorTest,OAuthProviderStrategyFactoryTest,UserQuotaGatewayTest,AuthControllerIntegrationTest,AuthVerificationControllerIntegrationTest,AuthRegistrationControllerIntegrationTest,AuthBindingControllerIntegrationTest,MeControllerIntegrationTest,AuthGrantStrategyFactoryTest,ImageCaptchaServiceTest -Dsurefire.failIfNoSpecifiedTests=false
```
...
- 验证码失败次数达到阈值后自动失效
- 新 auth 控制器路由与返回结构
- `me/account`、`me/profile`、`me/password` 路由与返回结构
...
4. 重构 user-module 认证入口为 grant 策略工厂。
