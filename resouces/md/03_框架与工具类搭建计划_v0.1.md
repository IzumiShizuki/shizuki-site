...
包含：
- 单体模块化工程结构搭建
- 配置入口打通（monolith.yaml + common-config）
- 入口鉴权能力（鉴权、限流、trace）
- 通用工具类与 Starter 落地
- 最小可运行链路与验收
...
1. 本地 `docker compose up` 后可启动：MySQL、Redis、Kafka、Monolith。
2. 单体入口可承接 user/content/media/ai 模块，具备统一鉴权与限流拦截能力。
3. 通用库被至少 2 个业务模块实际引用并生效。
...
1. 多模块 Maven 父工程
2. 模块清单（`apps/monolith-app`、`modules/{user,content,media,ai}-module`）
3. 公共模块（3 个）
4. `docker-compose` 基础环境
5. 单体配置模板（`resouces/yaml/monolith.yaml`、`resouces/yaml/common-config*.yaml`）
6. Phase0 验收文档与启动说明
...
### 4.1 `common-core`（所有模块共用的“通用语言层”）

这一层负责把“返回格式、错误模型、基础实体”统一起来，避免每个模块各说各话。
...
这一层主要解决“每个模块都要做，但不想每个模块都重复写”的事情：异常、请求链路、权限、审计、限流、安全净化。
...
| `libs/common-servlet/src/main/java/io/github/shizuki/common/security/filter/LoginUserContextFilter.java` | 从 Header 读取用户信息并放入上下文 | 已实现（入口透传兼容方案） |
...
- 当前代码已实现 Sa-Token 登录态闭环（user-module + monolith-app），并保留 Header 上下文透传作为模块内兼容机制。
...
| `libs/common-integration/src/main/java/io/github/shizuki/common/storage/client/AliyunOssClient.java` | 阿里云 OSS 适配实现 | 已实现并接入 media-module |
...
- 建立 parent `pom.xml`
- 固定依赖版本（Boot、MyBatis-Plus、Sa-Token）
- 配置统一 plugin（compiler/surefire/spotless/checkstyle）

### Step 2：基础中间件启动

- Compose 拉起 MySQL、Redis、Kafka
- 初始化数据库与用户
- 验证 monolith 可启动并读取配置
...
- Day 1：父工程 + Compose + 配置入口
- Day 2：monolith-app + user-module 骨架
- Day 3：common-core + common-servlet
- Day 4：common-integration + 联调修整
- Day 5：content-module 接入 + 联调 + 验收
...
1. 配置加载不一致
- 缓解：统一 `application.yml` 与 `resouces/yaml` 配置来源，并保留本地 fallback
