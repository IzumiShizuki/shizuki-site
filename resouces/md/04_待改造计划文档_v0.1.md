# 第二阶段：待改造计划文档（v0.1）

> 文档定位：记录本轮改造如何落地、已经完成什么、还需要继续优化什么。
> 更新时间：2026-02-12

---

## 0. 执行顺序（已按锁定顺序推进）

本轮按以下顺序执行：
- `1 + 3`：数据层持久化 + OSS 参数化与真实实现
- `5`：审计日志入库
- `2`：Sa-Token 鉴权闭环
- `4`：OAuth 真实闭环
- `6`：限流与配额升级

---

## 0.1 本轮增量修订（认证与韧性收口）

- 认证入口已从旧 `/api/v1/auth/login`、`/api/v1/oauth-logins/**` 收口到：
  - `/api/v1/auth/tokens`（统一 grant）
  - `/api/v1/auth/oauth/authorizations`
  - `/api/v1/auth/register/email`
  - `/api/v1/auth/bindings/confirm`
- 网关 guest 坏 token 策略已支持：
  - 默认 `downgrade`（降级游客）
  - 可切换 `reject`（严格拒绝）
- 网关 introspect 错误语义已固定：`4xx -> 401`，`timeout/5xx/network -> 503`，并输出结构化日志字段 `reason/upstream_status`。
- `UserQuotaClient` 已接入统一重试组件（指数退避 + jitter + cap），移除业务线程手写 `sleep`。
- 图形验证码新增失败次数上限，防止暴力尝试。
- 审计 outbox 已补 `PROCESSING` 超时回收，避免异常退出后事件长期卡死。
- JWT 密钥改为配置驱动，支持本地 `random.uuid` 兜底并保留启动告警。
- 已新增 `GUEST` 组 AI 配额策略：`ai_round_total = 0`。

---

## 1. 基础设施基线（单体配置 + 目录规范）

### 1.1 已实现

- 单体运行配置统一入口：
  - `apps/monolith-app/src/main/resources/application.yml`
  - `resouces/yaml/monolith.yaml`
  - `resouces/yaml/common-config.yaml`（私有）
  - `resouces/yaml/common-config.example.yaml`（模板）
- 新增 `resouces/yaml` 作为单体配置目录。
- 新增 `resouces/sql`，遵循“一个数据库一个 SQL 文件”的维护方式。

### 1.2 配置文件清单

- `resouces/yaml/monolith.yaml`
- `resouces/yaml/common-config.yaml`
- `resouces/yaml/common-config.example.yaml`
- `resouces/yaml/middleware-config.yaml`

### 1.3 SQL 文件清单与规范

- `resouces/sql/shizuki_user.sql`
- `resouces/sql/shizuki_content.sql`
- `resouces/sql/shizuki_media.sql`
- `resouces/sql/shizuki_ai.sql`

已执行规范：
- 每个文件头有数据库说明和 `last_modified_at`。
- 每个增量块有 `-- modified_at: yyyy-MM-dd HH:mm:ss`。
- 每个增量块有 `-- change: ...` 用途说明。
- DDL 大量使用 `IF NOT EXISTS` 保持可重复执行。
- 核心表与字段已补齐 `COMMENT` 注释，便于入库与后续维护。

### 1.4 仍待优化

- 已接入 Flyway 版本迁移；运行时迁移目录为 `apps/monolith-app/src/main/resources/monolith/db/migration`，仍需在真实环境补一次全量迁移演练与回滚演练。
- 已补对象命名规范脚本校验，后续需在 CI 中继续收敛历史脚本兼容规则。

---

## 2. 改造项完成情况

## 2.1 问题 1：数据层持久化（已完成）

### 已实现

- user-module：
  - 内存实现替换为 `UserServiceImpl`：
    - `modules/user-module/src/main/java/io/github/shizuki/site/user/service/impl/UserServiceImpl.java`
  - 新增实体与 Mapper：
    - `UserAccountEntity/UserPreferenceEntity/OAuthLoginEntity/OAuthBindingEntity/GroupQuotaPolicyEntity`
    - `UserAccountMapper/UserPreferenceMapper/OAuthLoginMapper/OAuthBindingMapper/GroupQuotaPolicyMapper`
- content-module：
  - 内存实现替换为 `ContentServiceImpl`。
  - 新增 `PostEntity/AppEntity/ContentReportEntity` 及对应 Mapper。
- media-module：
  - 内存实现替换为 `MediaServiceImpl`。
  - 新增 `MediaAssetEntity/MediaAssetReportEntity` 及对应 Mapper。
- ai-module：
  - 内存实现替换为 `AiServiceImpl`。
  - 新增 `AiSessionEntity/AiMessageEntity/AiQuotaUsageEntity/AiCharacterEntity` 及对应 Mapper。
- 4 个服务 `application.yml` 默认库已切换为分库：
  - `shizuki_user/shizuki_content/shizuki_media/shizuki_ai`。

### 仍待优化

- 当前仓库尚未补足完整集成测试（重启后数据一致性、事务边界、并发写入场景）。

---

## 2.2 问题 3：OSS 参数化与真实实现（已完成）

### 已实现

- `common-integration` 新增 OSS 参数对象：
  - `libs/common-integration/src/main/java/io/github/shizuki/common/storage/config/OssProperties.java`
- `AliyunOssClient` 已改为真实实现，支持：
  - 上传 `putObject`
  - 删除 `deleteObject`
  - 下载签名 URL `generateGetUrl`
  - 上传签名 URL `generatePutUrl`
- `media-module` 已接入真实对象存储客户端，不再使用 mock URL。
- `resouces/yaml/common-config.yaml` 写入 OSS 所需参数：`endpoint/bucket/sign-expire/max-upload-size` 等。

### 仍待优化

- OSS 密钥当前仍通过配置项注入，建议下一轮迁移到密钥管理系统（KMS/Vault）。

---

## 2.3 问题 5：审计日志入库（已完成）

### 已实现

- 每个分库都新增：
  - `audit_log`
  - `audit_event_outbox`
- 保留原 `AuditLogAspect` 采集能力，新增 DB 落库实现：
  - `libs/common-servlet/src/main/java/io/github/shizuki/common/audit/service/JdbcAuditLogService.java`
- DB 落库失败自动降级到日志实现：
  - `LoggingAuditLogService`
- 新增审计检索接口（支持 trace_id/user_id/action/time）：
  - `libs/common-servlet/src/main/java/io/github/shizuki/common/audit/controller/AuditLogQueryController.java`

### 仍待优化

- outbox 当前只做“结构预留 + 持久化”，未接实时消费链路（Kafka/ELK）。

---

## 2.4 问题 2：Sa-Token 鉴权闭环（已完成）

### 已实现

- 认证头统一：`Authorization: Bearer <token>`。
- user-module 新增登录闭环接口：
  - `POST /api/v1/auth/tokens`
  - `POST /api/v1/auth/password/reset`
  - `POST /api/v1/auth/logout`
  - `GET /api/v1/auth/introspect`
  - `POST /api/v1/auth/register/email`
  - `POST /api/v1/auth/verifications/email/send`
  - `GET /api/v1/auth/captchas/image`
  - `GET /api/v1/me/account`
  - `PUT /api/v1/me/profile`
  - `PUT /api/v1/me/password`
- monolith-app 新增统一鉴权过滤器：
  - `apps/monolith-app/src/main/java/io/github/shizuki/site/monolith/filter/AuthEntryFilter.java`
- 网关支持：
  - 白名单路径直接放行
  - guest 路径注入 GUEST 上下文
  - 受保护路径走 introspect 校验
  - 成功后透传 `X-User-Id/X-User-Groups/X-User-Permissions`

### 仍待优化

- 网关鉴权目前按路径匹配规则管理，建议下一轮补充“路由配置中心化 + 自动校验脚本”。

---

## 2.5 问题 4：OAuth 真实闭环（已完成）

### 已实现

- OAuth 入口已切换为：`/api/v1/auth/oauth/authorizations` + `/api/v1/auth/tokens(grant=oauth_code)`。
- `AuthFlowService` 完成真实 OAuth 链路：
  - create login
  - state 校验
  - code 换 token
  - 拉 GitHub/LinuxDo 用户信息
  - 本地账号绑定/创建
  - access JWT + refresh token 下发
- 防重复绑定通过 `(provider, provider_user_id)` 唯一键保障。
- 失败场景记录到 `oauth_login.status/error_message`。
- 邮箱冲突场景支持返回 `BIND_REQUIRED + bind_ticket`，并通过 `/api/v1/auth/bindings/confirm` 完成显式确认闭环。

### 仍待优化

- 建议补充 GitHub OAuth 全链路自动化测试（含 state 过期、重复登录、异常回调）。

---

## 2.6 问题 6：限流与配额升级（已完成）

### 已实现

- `RateLimitAspect` 键升级为用户/分组维度：
  - `rate-limit:<base>:u:<userId|guest>:g:<groups>`
- AI 配额改为数据库持久化计数：
  - `ai_quota_usage`
- AI 配额支持按用户分组动态解析：
  - ai-module 通过 `UserQuotaClient` 调 user-module 内部接口解析配额
  - user-module 通过 `group_quota_policy` 持久化配置返回结果
- 超限返回统一 ProblemDetail 结构，并携带 `remaining`：
  - 限流超限：`remaining=0`、`limit_key`、`window_seconds`
  - AI 配额耗尽：`remaining=0`、`quota_code`、`total`、`used`

### 仍待优化

- 限流当前为 Redis 优先 + 本地降级，下一轮建议补充分布式一致性压测与告警阈值。

---

## 3. 关键配置与脚本如何使用

## 3.1 SQL 初始化顺序

建议先执行：
1. `resouces/sql/shizuki_user.sql`
2. `resouces/sql/shizuki_content.sql`
3. `resouces/sql/shizuki_media.sql`
4. `resouces/sql/shizuki_ai.sql`

当前新增：
5. `resouces/sql/00_object_name_mapping.yaml`（旧名与新名映射、逻辑序列映射）

执行原则：
- 每次新增 SQL 直接追加到文件尾部。
- 必须写 `modified_at` 与 `change` 注释。
- 运行时以 Flyway 为准（`apps/monolith-app/src/main/resources/monolith/db/migration/V*.sql`），`resouces/sql` 作为人工导入与说明视图。

## 3.2 配置管理建议

建议配置文件分层：
1. `resouces/yaml/monolith.yaml`（公共运行配置）
2. `resouces/yaml/common-config.yaml`（私有敏感配置）
3. `resouces/yaml/common-config.example.yaml`（模板基线）

---

## 4. 本轮验收结果

### 4.1 已验证

- 多模块编译通过：
  - 执行命令：`mvn -DskipTests compile -T 1C`
  - 结果：9 个模块 `BUILD SUCCESS`
- 目录与配置基线已落地：`resouces/yaml`、`resouces/sql`、`application.yml`。
- 持久化、鉴权、OAuth、OSS、审计、配额六项改造代码已接入主干。
- 数据库迁移当前状态：
  - `monolith/db/migration`：`V101`~`V403`（user/content/media/ai 全量基线 + 增量）

### 4.2 未完成验证

- 端到端联调（依赖 MySQL/Redis/Kafka/OSS/GitHub OAuth 外部环境）。
- 自动化集成测试与压测报告。

---

## 5. 下一步待改进事项（建议）

1. Flyway 迁移已接入，下一步补齐真实环境迁移回归与 `flyway repair` 操作手册。
2. 增加 CI 校验：SQL 增量块必须包含 `modified_at` 和 `change` 注释。
3. 接入密钥托管（配置加密/KMS/Vault），移除业务配置中的明文敏感值。
4. 补齐鉴权、OAuth、配额、审计检索的集成测试。
5. 审计 outbox 接统一消费链路（Kafka/ELK）并补消费失败重试机制。

---

## 6. 下一步动工计划（建议）

1. 先动工测试与联调：补齐 `monolith/user/ai/media` 的集成测试，覆盖登录、OAuth、配额扣减、上传签名 URL。
2. 再动工数据治理：持续保持 `resouces/sql` 与 `apps/monolith-app/src/main/resources/monolith/db/migration` 同步，避免文档 SQL 与运行时迁移脚本漂移。
3. 最后动工审计链路：基于现有 `audit_event_outbox` 接入消费任务（先定时轮询，再演进 Kafka/ELK）。
