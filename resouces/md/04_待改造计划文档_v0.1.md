...
- 已接入 Flyway 版本迁移；当前仓库以 `V1/V2` 为主（`user-service` 额外有 `V3__auth_upgrade.sql`），仍需在真实环境补一次全量迁移演练与回滚演练。
...
- 运行时以 Flyway 为准（`services/*/db/migration/V*.sql`），`resouces/sql` 作为人工导入与说明视图。
...
- 数据库迁移当前状态：
  - `content-service`：`V1__init_schema.sql`、`V2__seed_data.sql`
  - `media-service`：`V1__init_schema.sql`、`V2__seed_data.sql`
  - `user-service`：`V1__init_schema.sql`、`V2__seed_data.sql`、`V3__auth_upgrade.sql`
...
3. 接入密钥托管（Nacos 加密/KMS/Vault），移除业务配置中的明文敏感值。
...
2. 再动工数据治理：持续保持 `resouces/sql` 与 `services/*/db/migration` 同步，避免文档 SQL 与运行时迁移脚本漂移。
