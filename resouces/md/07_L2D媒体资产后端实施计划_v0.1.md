...
- 文档日期：2026-02-12
- 适用仓库：`shizuki-site`
- 范围：后端一期（media-module/monolith-app/user-module）
...
- `media-module`
  - 新增资产类型枚举与可见性枚举。
  - 扩展上传策略与资产落库接口字段。
  - 新增分组 ACL 关联表（`MDA_ASSET_GROUP_ACL`）。
  - 新增公开首页角色池接口。
...
- `monolith-app`
  ...
  - 管理路由新增 `/api/v1/admin/users/**`、`/api/v1/admin/groups/**`、`/api/v1/admin/posts/**`、`/api/v1/admin/apps/**`。
- `user-module`
...
- `visibility_code TINYINT`：1 私有 / 2 公开 / 3 分组
...
### 3.2 MDA_ASSET_GROUP_ACL 新表

- `asset_id`（关联 `MDA_ASSET.id`）
- `group_code`（授权分组编码，统一大写）
- 唯一约束：`(asset_id, group_code)`

### 3.3 MDA_L2D_PACKAGE 新表
...
```mermaid
erDiagram
  MDA_ASSET ||--o{ MDA_ASSET_GROUP_ACL : "asset_id"
...
1. 本轮媒体三态可见性与 ACL 仅修改 `V1__init_schema.sql`（扩展 `visibility_code` 语义并新增 `MDA_ASSET_GROUP_ACL`）。
2. 本需求不新增 `V3/V4` 迁移文件。
3. 本地若已执行过旧基线，默认重建数据库后重新执行 Flyway。
4. `resouces/sql/shizuki_media.sql` 保持与 `V1` 一致。
...
- `content_type`
- `visibility`
- `allowed_group_codes`（可选，`visibility=GROUP` 时必填）
- `metadata`（可选）
...
- `is_public`
- 公开资源返回 `public_url`
- 非公开资源返回 `download_url + expire_seconds`
- 访问规则：
  - `PUBLIC + APPROVED` 可匿名访问
  - `GROUP` 需 owner / ADMIN / 命中任一 ACL 分组
  - 其余仅 owner 或 ADMIN 可访问
...
- `home_sort_num`
- `visibility`
- `allowed_group_codes`（仅 `visibility=GROUP` 时允许传入）
...
```bash
mvn -pl modules/media-module,apps/monolith-app,modules/user-module -am test -DskipITs
```
...
为与 `user-module` 风格保持一致，本期后端实现采用以下注释约定：
...
2. 对以下高风险逻辑补行内注释：
   ...
   - 可见性与 bucket 绑定校验（避免公开/私有/分组策略绕过）
3. 新增 `enum/dto/entity` 的关键字段补语义注释，强调 code 与字符串映射关系。
4. 后续功能扩展时，保持“改逻辑必改注释”的同步原则。

## 9. 下一步（前端对接准备）

1. 前端上传时必须显式传 `asset_kind`、`content_type`、`visibility`；当 `visibility=GROUP` 时必须传 `allowed_group_codes`。
