...
public abstract class a {
    ...
    public static int a(double d10, double d11, double d12) {
        ...
        int iMin = iRound < 0 ? 0 : Math.min(iRound, TinkerReport.KEY_LOADED_EXCEPTION_RESOURCE_CHECK);
        int iRound2 = (int) Math.round(dPow2 * 255.0d);
        int iMin2 = iRound2 < 0 ? 0 : Math.min(iRound2, TinkerReport.KEY_LOADED_EXCEPTION_RESOURCE_CHECK);
        int iRound3 = (int) Math.round(dPow3 * 255.0d);
        return Color.rgb(iMin, iMin2, iRound3 >= 0 ? Math.min(iRound3, TinkerReport.KEY_LOADED_EXCEPTION_RESOURCE_CHECK) : 0);
    }
}
