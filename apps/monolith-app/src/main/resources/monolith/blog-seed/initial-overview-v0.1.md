...
2. 架构采用 **单体 + 模块化（Monolith Modular）**：`apps/monolith-app` + `modules/*` + `libs/common-*`。
3. 技术基线：`Java 17 + Spring Boot 3.x + MyBatis-Plus + MySQL + Redis + Kafka + OSS + Sa-Token`。
...
7. 部署目标为单机 8GB：单体应用与中间件可控运行，预留约 1GB 内存缓冲。
...
- 可维护：强制统一命名与错误模型，避免模块间风格漂移。
- 可止损：入口限流 + 模块配额 + 维护开关三层保护。
- 可降本：媒体对象存 OSS，计算留给业务模块，首期不引入重型组件。
...
- 配额：
  - 无自带 API：总 5 轮。
  - 有自带 API：不限总轮数。
- 保护：入口限流 30/min + ai-module 二次配额校验。
- 数据保留：会话历史默认 30 天，可手动清理。
- 角色卡：支持 `character` 与 `character_card_png` 两种资产形态。

### 4.4 内容与媒体可见性（当前实现）

- 可见性统一三态：`PUBLIC` / `PRIVATE` / `GROUP`。
- `PRIVATE`：仅 owner 与 `ADMIN` 可访问。
- `GROUP`：owner、`ADMIN` 或命中任一授权分组可访问。
- 生效点：内容列表（`GET /api/v1/posts`、`GET /api/v1/apps`）与点赞接口（`POST /api/v1/posts/{post_id}/likes`、`POST /api/v1/apps/{app_id}/likes`）都必须执行可见性校验。
...
1. `apps/monolith-app`：单体入口、统一鉴权、统一限流、统一日志透传。
2. `modules/{user,content,media,ai}-module`：业务能力模块（用户、内容、媒体、AI）。
3. `libs/common-*`：公共能力（core/servlet/integration）。

### 5.2 配置入口

- 运行配置入口：`resouces/yaml/monolith.yaml`。
- 私有配置入口：`resouces/yaml/common-config.yaml`（示例模板：`common-config.example.yaml`）。

### 5.3 通信与容错

- 模块间调用：以单体内服务编排为主，保留外部系统 HTTP 调用适配层。
- 容错：超时、重试、熔断（首期以入口限流 + 调用超时为主）。
- Trace：统一 `trace_id` 贯穿入口与业务模块。
...
- 入口访问日志：`path/status/cost_ms/user_id/trace_id`。
...
| 服务 | JVM 建议 | 预计 RSS |
|---|---|---|
| monolith-app | `-Xms1536m -Xmx1536m` | 1.90 GB |
...
触发后优先扩容：`monolith-app -> MySQL`。

---

## 10. 迭代路线

- Phase 0：框架骨架、入口鉴权、审计、统一异常。
...
- 配置入口已统一：`resouces/yaml/monolith.yaml` + `resouces/yaml/common-config.yaml`。
- 数据层已切换为 MyBatis-Plus 持久化实现，服务命名统一为 `Service/ServiceImpl`。
- OAuth 已打通真实链路（state 校验、code 换 token、userinfo、本地绑定/建号、登录态下发）。
- OSS 已完成真实 SDK 适配，`media-module` 已使用签名 URL 上传/下载策略。
