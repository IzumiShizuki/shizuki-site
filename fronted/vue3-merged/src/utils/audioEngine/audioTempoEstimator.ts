 ...
 * 原路径: src/renderer/src/components/audioTempoEstimator.ts
 * 移植说明: 逐字移植（算法零桌面依赖）。Web 端输入适配：
 * visualizerBars ← AnalyserNode 频谱降维、waveform ← 时域数据、
 * rmsDb ← 时域 RMS 估算，由上层喂入 TempoFrameInput。
 */
    ...
    for (let index = 1; index < this.samples.length - 1; index += 1) {
      ...
      if (previousPeak !== undefined && timestamp - previousPeak < minPeakSpacingMs) {
        if (strength > (peakStrengths[peakStrengths.length - 1] ?? 0)) {
          ...
          peakStrengths[peaks.length - 1] = strength
        }
        continue
      }
