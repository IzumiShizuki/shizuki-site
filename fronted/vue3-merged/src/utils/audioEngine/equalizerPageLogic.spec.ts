 ...
 * 原路径: src/renderer/src/utils/equalizerPageLogic.test.ts
 * 移植说明: 测试运行器 node:test → vitest；断言库保持 node:assert/strict。
 */
import assert from 'node:assert/strict'
import { test } from 'vitest'
import type { EqualizerBand, HeadphoneCompensationSettings } from './equalizerTypes'
import {
