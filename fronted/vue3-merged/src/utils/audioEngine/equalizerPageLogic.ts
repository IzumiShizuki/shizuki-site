 ...
 * 原路径: src/renderer/src/utils/equalizerPageLogic.ts
 * 移植说明:
 * - 类型 import 改为本地 ./equalizerTypes；
 * - defaultAudioProcessing / normalizeAudioProcessing 剔除 DSD、卷积、
 *   输出设备等 C 级字段，仅保留 EQ 子集（dspEnabled 语义保留）；
 * - tabs 的 PrimeIcons 图标类替换为站点 FontAwesome（fa-solid fa-*）。
 */
import type {
  AudioEqPreset,
