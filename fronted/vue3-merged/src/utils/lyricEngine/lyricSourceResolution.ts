 ...
 * Original author: Px-asen.
 * Modified for shizuki-site: the Electron `Track` / `MediaProviderLyrics` /
 * `getNcmSongId` imports are replaced by minimal local bridge declarations in
 * `./types.ts`; the source-resolution algorithm itself is unchanged.
 */

import type { LyricSource } from './lyricsManagement.ts'
import { getNcmSongId, type MediaProviderLyrics, type Track } from './types.ts'
