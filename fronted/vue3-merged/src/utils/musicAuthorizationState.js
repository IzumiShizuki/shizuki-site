export const SOURCE_ACCOUNT_PROVIDERS = Object.freeze([
  "netease",
  "qqmusic",
  "kugou",
]);
...
export function normalizeSourceAccountStatus(raw, fallbackProvider = "") {
  const provider = String(raw?.provider || fallbackProvider || "")
    .trim()
    .toLowerCase();
  return {
    provider,
    authType:
      String(raw?.authType || raw?.auth_type || "cookie").trim() || "cookie",
    ...
    status:
      String(raw?.status || "")
        .trim()
        .toUpperCase() ||
      (Boolean(raw?.bound ?? raw?.keyBound ?? raw?.key_bound)
        ? "BOUND"
        : "UNBOUND"),
    updatedAt: String(raw?.updatedAt || raw?.updated_at || "").trim(),
    lastVerifiedAt: String(
      raw?.lastVerifiedAt || raw?.last_verified_at || "",
    ).trim(),
