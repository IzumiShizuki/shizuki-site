 ...
 * Original: src/renderer/src/components/player-bar/usePlaybackQueueDrawerActions.test.ts
 * (node:test → vitest)
 */
...
import { usePlaybackQueueDrawerActions } from './usePlaybackQueueDrawerActions.js';
...
describe('usePlaybackQueueDrawerActions', () => {
  it('virtual queue drawer resolves drag source and target by stable IDs after its visible window shifts', () => {
    ...
    expect(reorders).toEqual([[9_999, 10_004]]);
    expect(actions.draggedEntryId.value).toBeNull();
  });

  it('drawer queue commands target duplicate tracks by entry identity and save a named snapshot', () => {
    ...
    expect(actions.saveAsPlaylist('  My Queue  ')).toBe('playlist-id');
    actions.clear();
    ...
    expect(actions.saveAsPlaylist('   ')).toBeNull();
  });
});
