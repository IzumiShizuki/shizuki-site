<!--
  ...
  原路径: src/renderer/src/components/equalizer/ParametricEqWorkspace.vue
  移植说明: 纯 props/emits 参数 EQ 工作台；PrimeIcons（pi pi-*）替换为站点
  FontAwesome（fa-solid fa-*）；@renderer 别名改为相对路径；
  --te-* / --eq-* 主题变量映射到站点 --theme-*/--accent-* 变量；
  html[data-te-*]、html[data-theme] 主题 hook 删除（站点无该属性）。
-->
...
import {
  ...
  percentToFrequency,
  percentToGain
} from '../../../utils/audioEngine/parametricEqInteraction'
import type { EqualizerBand, EqualizerFilterType } from '../../../utils/audioEngine/equalizerTypes'
...
<template>
  <section class="parametric-workspace">
...
/* ————————————————————————————————————————————————
   ...
   站点适配：--eq-* 直接映射到站点 --theme-*/--accent-* 变量
   （站点无 html[data-theme] 属性，暗色随站点主题变量自动切换）。
———————————————————————————————————————————————— */
.parametric-workspace {
  --eq-surface: var(--theme-panel-surface);
  --eq-surface-soft: var(--theme-surface-soft);
  --eq-panel: var(--theme-surface-elevated);
  --eq-panel-raised: var(--theme-panel-surface-elevated);
  --eq-text: var(--theme-text-primary);
  --eq-text-muted: var(--theme-text-secondary);
  --eq-text-subtle: var(--theme-text-tertiary);
  --eq-border: var(--theme-border-strong);
  --eq-border-soft: var(--theme-border-subtle);
  --eq-grid: color-mix(in srgb, var(--theme-text-primary) 7%, transparent);
  --eq-grid-major: color-mix(in srgb, var(--theme-text-primary) 12%, transparent);
  --eq-zero-axis: color-mix(in srgb, var(--theme-warning) 48%, transparent);
  --eq-response: var(--theme-text-primary);
  --eq-composite: rgb(var(--accent-rgb));
  --eq-accent: rgb(var(--accent-rgb));
  --eq-source: #38bdf8;
  --eq-target: var(--theme-text-tertiary);
  --eq-filter-combined: var(--theme-warning);
  --eq-corrected: var(--theme-positive);
  --eq-spectrum: color-mix(in srgb, var(--theme-text-primary) 38%, transparent);
  --eq-control-bg: color-mix(in srgb, var(--theme-text-primary) 5%, transparent);
  --eq-tooltip-bg: var(--theme-floating-surface);
  --eq-meter: var(--theme-positive);
