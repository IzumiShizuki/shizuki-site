            ...
            <template #section-music-auth>
              <div class="music-auth-grid">
                <article class="music-auth-card">
                  ...
                  <p class="helper-text">用于 Meting 聚合能力，默认推荐会优先使用该 Key。</p>
                  <div class="stack-form">
                    ...
                    />
                    <div class="inline-actions">
                      <button class="ghost-btn ripple-trigger" type="button" @click="openMetingConsolePage">
                        打开配置页
                      </button>
...
async function saveMusicTunehubKey() {
  if (!String(musicTunehubKeyInput.value || '').trim()) {
    musicPreferenceError.value = '请输入 Meting API Key';
    return;
  }
  musicTunehubBusy.value = true;
  musicPreferenceError.value = '';
  try {
    const payload = await musicApi.upsertMusicApiKey(METING_API_PROVIDER, musicTunehubKeyInput.value, auth.authorizedFetch);
    musicTunehubStatus.value = normalizeApiKeyStatus(payload);
    musicTunehubKeyInput.value = '';
    setGlobalHint('Meting Key 已更新');
  } catch (error) {
    musicPreferenceError.value = readErrorMessage(error);
  } finally {
...
const musicTunehubStatusText = computed(() => {
  if (!auth.isAuthenticated.value) return '登录后可保存 Meting Key';
