...
wsl.exe bash -lc "set -e; APP_DIR=\$(wslpath -a \"%~dp0..\"); cd \"\$APP_DIR\"; export REMOTE_HOST='%REMOTE_HOST%'; export REMOTE_USER='%REMOTE_USER%'; export REMOTE_PASS='%REMOTE_PASS%'; export REMOTE_APP_DIR='%REMOTE_APP_DIR%'; export REMOTE_DEPLOY_DIR='%REMOTE_DEPLOY_DIR%'; export UPLOAD_RETRIES='%UPLOAD_RETRIES%'; bash deploy/update-code-and-deploy.sh"
if errorlevel 1 (
